...
Environment network policies use Kubernetes `NetworkPolicy` for IP/CIDR rules. Domain allowlists require a supported FQDN policy provider such as Cilium `CiliumNetworkPolicy`, GKE `FQDNNetworkPolicy`, or EKS Auto Mode `ApplicationNetworkPolicy`; grant the Helm service account the chart's default MCP manager RBAC so Archestra can create the detected object types where available. AWS DNS-based rules only apply to workloads running on EKS Auto Mode-launched EC2 instances. See Kubernetes [Network Policies](https://kubernetes.io/docs/concepts/services-networking/network-policies/), Cilium [DNS-based policies](https://docs.cilium.io/en/latest/security/dns/), GKE [FQDN network policies](https://cloud.google.com/kubernetes-engine/docs/how-to/fqdn-network-policies), and EKS Auto Mode [network policies](https://docs.aws.amazon.com/eks/latest/userguide/auto-net-pol.html).
