...
Environment network policies use Kubernetes `NetworkPolicy` for IP/CIDR rules. Domain allowlists require Cilium's `CiliumNetworkPolicy`; grant the Helm service account the chart's default MCP manager RBAC so Archestra can create both object types where available. Kubernetes documents the base API in [Network Policies](https://kubernetes.io/docs/concepts/services-networking/network-policies/), and Cilium documents FQDN policy in [DNS-based policies](https://docs.cilium.io/en/latest/security/dns/).
