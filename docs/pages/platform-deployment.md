...
Environment network policies use Kubernetes `NetworkPolicy` for IP/CIDR rules. Domain allowlists require a supported FQDN policy provider such as Cilium `CiliumNetworkPolicy` or GKE `FQDNNetworkPolicy`; grant the Helm service account the chart's default MCP manager RBAC so Archestra can create the detected object types where available. See Kubernetes [Network Policies](https://kubernetes.io/docs/concepts/services-networking/network-policies/), Cilium [DNS-based policies](https://docs.cilium.io/en/latest/security/dns/), and GKE [FQDN network policies](https://cloud.google.com/kubernetes-engine/docs/how-to/fqdn-network-policies).
